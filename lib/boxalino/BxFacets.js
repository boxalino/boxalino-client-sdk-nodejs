(function (factory) {
    ...
    var BxFacets = /** @class */ (function () {
        ...
        BxFacets.prototype.buildTree = function (response, parents, parentLevel) {
            ...
            if (parents.length == 0) {
                ...
                if (parents.length == 1) {
                    ...
                    root['selected'] = false;
                    tree = { 'node': root, 'children': children_1 };
                }
            }
            var children = Array();
            response.forEach(function (node) {
        ...
        BxFacets.prototype.getFacetsAsObjectsCollection = function (language) {
            if (language === void 0) { language = null; }
            ...
            if (this.searchResult != null && this.searchResult.facetResponses != null) {
                var Obj_1 = this;
                this.searchResult.facetResponses.forEach(function (facetResponse) {
                    var facetField = facetResponse.fieldName;
                    var type = Obj_1.getFacetType(facetField);
                    switch (type) {
                        case 'hierarchical': {
                            //statements;
                            properties = Obj_1.prepareFacetsValuesForHierarchicalType(facetResponse, 0);
                            break;
                        }
                        case 'ranged': {
                            //statements;
                            properties = Obj_1.prepareFacetsValuesForRangedType(facetResponse);
                            break;
                        }
                        default: {
                            //statements;
                            properties = Obj_1.prepareFacetsValuesForDefaultType(facetResponse);
                            break;
                        }
                    }
                    var overWriteRanking = Obj_1.getFacetExtraInfo(facetField, "valueorderEnums");
                    properties = Obj_1.prepareFacetsByDisplaySelectedValuesExtra(facetField, properties);
                    properties = Obj_1.applyDependencies(facetField, properties);
                    properties = Obj_1.prepareFacetsByEnumDisplaySizeExtra(facetField, properties);
                    properties = Obj_1.prepareFacetsByIconMapExtra(properties, facetField);
                    var hasValueCorrelation = Obj_1.getFacetExtraInfo(facetField, "facet-value-correlation");
                    if (hasValueCorrelation != null) {
                        properties = Obj_1.prepareFacetsByValueCorrelation(facetField, properties, hasValueCorrelation);
                    }
                    var facetObject = {
                        optionValues: properties,
                        fieldName: facetField,
                        order: facetResponse.sortOrder,
                        label: Obj_1.getFacetLabel(facetField, language),
                        showCounter: Obj_1.showFacetValueCounters(facetField),
                        displayType: Obj_1.getFacetDisplay(facetField),
                        hidden: String(Obj_1.getFacetDisplay(facetField)) == "hidden" ? true : false,
                        type: Obj_1.getFacetType(facetField),
                        icon: Obj_1.getFacetIcon(facetField)
                    };
        ...
        BxFacets.prototype.prepareFacetsByValueCorrelation = function (fieldName, facetValues, valueCorrelationField) {
            var extraValuesInfo = this.getFacetExtraInfo(fieldName, valueCorrelationField);
            if (extraValuesInfo == null) {
                return facetValues;
            }
            var extraValues = JSON.parse(extraValuesInfo);
            var finalFacetValues = Array();
        ...
        BxFacets.prototype.prepareFacetsByIconMapExtra = function (facetValues, fieldName, language, defaultValue) {
            if (language === void 0) { language = null; }
            if (defaultValue === void 0) { defaultValue = ''; }
            var facetIconMap = this.getFacetExtraInfo(fieldName, 'iconMap');
            if (facetIconMap == null) {
                return facetValues;
            }
            var iconMap = JSON.parse(facetIconMap);
            var _loop_1 = function (name_1) {
                var properties = facetValues[name_1];
                var facetValue = name_1.toLowerCase();
                iconMap.forEach(function (icon) {
                    /*if(language && icon.language != language) {
                        continue;
                    }*/
                    if (facetValue == icon.value.toLowerCase()) {
                        ...
                        facetValues[name_1] = properties;
                    }
                });
                return { value: facetValues };
            };
            for (var name_1 in facetValues) {
        ...
        BxFacets.prototype.prepareFacetsByEnumDisplaySizeExtra = function (fieldName, facetValues) {
            var enumDisplaySize = parseInt(this.getFacetExtraInfo(fieldName, "enumDisplayMaxSize"));
        ...
        BxFacets.prototype.prepareFacetsByDisplaySelectedValuesExtra = function (fieldName, facetValues) {
            var displaySelectedValues = this.getFacetExtraInfo(fieldName, "displaySelectedValues");
            var finalFacetValues = Array();
            if (displaySelectedValues == "only") {
                for (var k in facetValues) {
        ...
        BxFacets.prototype.prepareFacetsValuesForDefaultType = function (facetResponse) {
            var facetValues = Array();
            ...
            var fieldName = facetResponse.fieldName;
            if (this.searchResult && this.facets.length != this.searchResult.facetResponses.length) {
        ...
        BxFacets.prototype.prepareFacetsValuesForRangedType = function (facetResponse) {
            var facetValues = [];
            var displayRange = "";
            if (this.getFacetExtraInfo(facetResponse.fieldName, 'bx_displayPriceRange') != null) {
                var displayRange_2 = JSON.parse(this.getFacetExtraInfo(facetResponse.fieldName, 'bx_displayPriceRange'));
            }
            facetResponse.values.forEach(function (facetValue) {
                if (displayRange) {
        ...
        BxFacets.prototype.prepareFacetsValuesForHierarchicalType = function (facetResponse, minCategoryLevel) {
            var facetValues = [];
            var tree = this.buildTree(facetResponse.values);
            tree = this.getSelectedTreeNode(tree);
            var node = this.getFirstNodeWithSeveralChildren(tree, minCategoryLevel);
