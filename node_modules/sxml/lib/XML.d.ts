import * as std from "tstl";
import { XMLList } from "./XMLList";
export declare class XML extends std.HashMap<string, XMLList> {
    /**
     * @hidden
     */
    private tag_;
    /**
     * @hidden
     */
    private value_;
    /**
     * @hidden
     */
    private property_map_;
    constructor();
    constructor(str: string);
    constructor(xml: XML);
    /**
     * @hidden
     */
    private _Copy_constructor;
    /**
     * @hidden
     */
    private _Parser_constructor;
    /**
     * @hidden
     */
    private _Parse;
    /**
     * @hidden
     */
    private _Parse_tag;
    /**
     * @hidden
     */
    private _Parse_properties;
    /**
     * @hidden
     */
    private _Parse_value;
    /**
     * @hidden
     */
    private _Parse_children;
    getTag(): string;
    getValue(): string;
    findProperty(key: string): std.HashMap.Iterator<string, string>;
    hasProperty(key: string): boolean;
    getProperty(key: string): string;
    getPropertyMap(): std.HashMap<string, string>;
    setTag(val: string): void;
    setValue(val: string): void;
    insertValue(tag: string, value: string): XML;
    setProperty(key: string, value: string): void;
    eraseProperty(key: string): void;
    push(...args: std.IPair<string, XMLList>[]): number;
    push(...xmls: XML[]): number;
    push(...xmlLists: XMLList[]): number;
    protected _Handle_insert(first: std.HashMap.Iterator<string, XMLList>, last: std.HashMap.Iterator<string, XMLList>): void;
    toString(tab?: number): string;
    /**
     * @hidden
     */
    private static _Compute_min_index;
    /**
     * @hidden
     */
    private static _Repeat;
}
export declare namespace XML {
    type Iterator = std.HashMap.Iterator<string, XMLList>;
    type ReverseIterator = std.HashMap.ReverseIterator<string, XMLList>;
    type iterator = Iterator;
    type reverse_iterator = ReverseIterator;
    function head(encoding?: string): string;
    function encode_value(str: string): string;
    function encode_property(str: string): string;
    function decode_value(str: string): string;
    function decode_property(str: string): string;
}
//# sourceMappingURL=XML.d.ts.map