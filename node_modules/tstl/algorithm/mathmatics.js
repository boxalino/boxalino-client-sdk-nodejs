...
function is_permutation(first1, last1, first2, pred) {
    ...
    // find the mismatched
    var pair = iterations_1.mismatch(first1, last1, first2);
    first1 = pair.first;
    first2 = pair.second;
    ...
    var _loop_1 = function (it) {
        ...
        if (iterations_1.find_if(first1, it, lamda).equals(it)) {
            var n = iterations_1.count(first2, last2, it.value);
            if (n === 0 || iterations_1.count(it, last1, it.value) !== n)
                return { value: false };
        }
    };
...
function next_permutation(first, last, compare) {
    if (compare === void 0) { compare = comparators_1.less; }
    if (first.equals(last) === true)
        return false;
    var i = last.prev();
    if (first.equals(i) === true)
...
//# sourceMappingURL=mathmatics.js.map