import { _HashBuckets } from "./_HashBuckets";
import { SetContainer } from "../container/SetContainer";
import { IHashSet } from "../container/IHashSet";
import { SetIterator } from "../iterator/SetIterator";
/**
 * @hidden
 */
export declare class _SetHashBuckets<T, Unique extends boolean, Source extends SetContainer<T, Unique, Source>> extends _HashBuckets<SetIterator<T, Unique, Source>> {
    private source_;
    private hash_function_;
    private key_eq_;
    constructor(source: IHashSet<T, Unique, Source>, hash: (val: T) => number, pred: (x: T, y: T) => boolean);
    hash_function(): (val: T) => number;
    key_eq(): (x: T, y: T) => boolean;
    find(val: T): SetIterator<T, Unique, Source>;
    hash_index(it: SetIterator<T, Unique, Source>): number;
}
//# sourceMappingURL=_SetHashBuckets.d.ts.map